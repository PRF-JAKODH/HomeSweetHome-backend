...
public class NotificationController {
     ...
     **/
    @GetMapping("/test/{id}")
    public void testMessage(@PathVariable Long id) {
        var notification = OrderNotification.OrderCompleted.builder()
                .userName("test")
                .orderId(12345L)
                .build();
        Long randomId = ThreadLocalRandom.current().nextLong(1, id);
        notificationSendService.sendTemplateNotificationToSingleUser(randomId, notification);
    }

    @GetMapping("/test/multiple/{range}")
    public void testMessageMultiple(@PathVariable int range) {
        var notification = OrderNotification.OrderCompleted.builder()
                .userName("test")
                .orderId(12345L)
                .build();
        List<Long> ids = LongStream.rangeClosed(1, range).boxed().toList();
        notificationSendService.sendTemplateNotificationToMultipleUsers(ids, notification);
    }
