...
/**
 * 🔥 현실적인 데드락 유발 테스트: "Viral Post Spike"
 *
 * 목표: 1분 안에 트래픽이 급증하는 상황에서
 *       조회/좋아요/댓글이 뒤섞일 때 발생하는 데드락(Deadlock) 감지
 */

// ==================== 📊 메트릭 설정 ====================
const deadlockCounter = new Counter('deadlock_count'); // 데드락 발생 횟수 (핵심)
const errorRate = new Rate('error_rate');              // 전체 에러율
const reqDuration = new Trend('req_duration');         // 응답 속도 추적

// ==================== ⚙️ 테스트 설정 ====================
export const options = {
    scenarios: {
        // 1분 급상승 시나리오 (Spike Test)
        viral_spike: {
...
export function viralAction() {
    ...
    /**
     * 🎲 행동 패턴 확률 분포 (현실성 부여)
     * - 60%: 조회수 증가 (가장 빈번) -> Update
     * - 20%: 좋아요 토글 (중간 빈도) -> Insert/Delete + Update
     * - 20%: 댓글 작성 (무거운 작업) -> Insert + Update
     *
     * 섞여서 들어와야 Lock 종류가 꼬이면서 데드락이 잘 터짐.
     */
...
export function handleSummary(data) {
    ...
    return {
        'stdout': `
    ==========================================================
    🔥  VIRAL POST SPIKE TEST RESULT (1 Minute)  🔥
    ==========================================================
    ...
    💀 데드락(Deadlock) 현황
    ----------------------------------------------------------
    발생 횟수 : ${deadlocks} 건  ${deadlocks > 0 ? "⛔ (위험! 로직 수정 필요)" : "✅ (안전함)"}
    실패율    : ${failRate} %

    📋 결론
    ----------------------------------------------------------
    ${deadlocks > 0
            ? "❌ 테스트 실패: 데드락이 감지되었습니다.\n    -> JPQL Atomic Update 적용 또는 쿼리 순서 점검 요망."
            : "🎉 테스트 성공: 트래픽 폭주 상황에서도 데드락이 발생하지 않았습니다."}
    ==========================================================
    `,
    };
}